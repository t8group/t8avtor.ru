...
<body>
<pre>
Полное имя:    {$_REQUEST['fullName']}
Email:         {$_REQUEST['email']}
Телефон:       {$_REQUEST['phone']}
Страна, Город: {$_REQUEST['location']}
</pre>
</body>
</html>
EOHD;
...
for (;;) {
    /* @var SimpleMail $mail */
    $mail = new SimpleMail();
    $mail->setTo('fedor.troitsky@gmail.com', 'John Smith')
        ->setSubject('Just Test Message')
        ->setFrom('contestform@t8avtor.ru', 't8avtor.ru')
        ->addMailHeader('Reply-To', 'no-reply@t8avtor.ru', 't8avtor.ru')
        ->setMessage($message)
        ->setWrap(80);
...
var_export([message, $headers]);die();
